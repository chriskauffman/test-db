...
class EntityKeyValueCommandSet(BaseCommandSet):
    ...
    @cmd2.with_argparser(tdb_entity_key_value_add_parser)
    def do_tdb_entity_key_value_add(self, args):
    ...
    @cmd2.with_argparser(tdb_entity_key_value_delete_parser)
    def do_tdb_entity_key_value_delete(self, args):
        entity = self.validate_entity(args.entity_gid)
    ...
    @cmd2.with_argparser(tdb_entity_key_value_view_parser)
    def do_tdb_entity_key_value_view(self, args):
        entity = self.validate_entity(args.entity_gid)
        key_value = entity.getKeyValueByKey(args.key)
        if key_value:
