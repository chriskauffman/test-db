...
person_key_value_app = typer.Typer()


@person_key_value_app.command("add")
def person_key_value_add(person_gid: str, key: str, value: str):
    person = validate_person(person_gid)
    try:
        key_value = person.getKeyValueByKey(key, itemValue=value)
...
@person_key_value_app.command("delete")
def person_key_value_delete(person_gid: str, key: str):
    person = validate_person(person_gid)
...
@person_key_value_app.command("view")
def person_key_value_view(person_gid: str, key: str):
    person = validate_person(person_gid)
